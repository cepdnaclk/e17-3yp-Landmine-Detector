...
const cache = {};
const defaultTemplate = path_1.join(__dirname, '../../public/serve-explorer', 'explorer.html');
const defaultStylesheet = path_1.join(__dirname, '../../public/serve-explorer', 'style.css');
...
const explorer = (root, options) => {
    ...
    const rootPath = path_1.normalize(resolve(root) + path_1.sep);
    ...
    return function (req, res, next) {
        ...
        const path = path_1.normalize(path_1.join(rootPath, dir));
        ...
        // determine ".." display
        const showUp = path_1.normalize(resolve(path) + path_1.sep) !== rootPath;
        ...
        fs_1.default.stat(path, function (err, stat) {
            ...
            if (err) {
                ...
                return next(Object.assign(Object.assign({}, err), { status: err.code }));
            }
            if (!stat.isDirectory())
                return next();
            // fetch files
...
function createHtmlFileList(files, dir, useIcons, view) {
    ...
    html += files
        ...
        let href = escape_html_1.default(normalizeSlashes(path_1.normalize(path.join('/'))));
        // use serve-preview
        ...
        return ('<li><a href="' +
            href +
            '" class="' +
            escape_html_1.default(classes.join(' ')) +
            '"' +
            ' title="' +
            escape_html_1.default(file.name) +
            '">' +
            '<span class="name">' +
            escape_html_1.default(file.name) +
            '</span>' +
    ...
    return function render(locals, callback) {
        ...
        fs_1.default.readFile(template, 'utf8', function (err, str) {
            ...
            const body = str
                ...
                .replace(/\{directory\}/g, escape_html_1.default(locals.directory))
...
function htmlPath(dir) {
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (part) {
            ...
            crumb[i] = '<a href="' + escape_html_1.default(parts.slice(0, i + 1).join('/')) + '">' + escape_html_1.default(part) + '</a>';
        }
    }
    return crumb.join(' / ');
}
...
function stat(dir, files, cb) {
    ...
    files.forEach(function (file) {
        batch.push(function (done) {
            fs_1.default.stat(path_1.join(dir, file), function (err, stat) {
                if (err && err.code !== 'ENOENT')
                    return done(err);
